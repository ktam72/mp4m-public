...
- **マルチコア専用スレッドオフロード**: Apple Silicon M1/M2/M3のPerformanceコアを活用し、MDX/PDXデコード・音声レンダリングを`Task.detached(priority: .userInitiated)`による高優先度バックグラウンドスレッドに移譲。メインスレッドのUI処理負荷を軽減し、60fpsの応答性を維持。
