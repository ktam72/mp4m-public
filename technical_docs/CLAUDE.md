...
## 2026-05-30 〜 KNA03.MDX ymfm FMパーカッション曲頭不具合の調査と妥協

**対象曲**: KNA03.MDX（Knight Arms / Arsys）

**現象**:
- ymfm使用時、曲頭のFM音源によるリズムパーカッションが正常に鳴らない（特に最初の打撃が弱い・欠ける）。
- fmgenでは問題なく再生される。
- A-2（初回音色不良対策）以前から存在し、A-2を無効化しても発生する。

**調査内容**:
- ビルド＋ログ収集スクリプトを整備。
- `MP4M_YMFM_DEBUG=1` + `HIGHRES=1` で高解像度チャンネル状態ログを取得。
- SetData直後の初期状態を12連発でダンプする `[YMFM_CH_INIT]` / `[FMGEN_CH_INIT]` を実装し、両エンジンで同じ曲を同じ長さ再生して比較。
- コードレベルでエンベロープ初期化・KeyOn直後の処理を詳細比較（reset後のattenuation/state、start_attack、attack時の初動計算、高レートショートカットなど）。

**主な発見**:
- チャンネル状態（KeyOn/KC/volume）の推移は ymfm と fmgen で非常に似ている。
- 差は OPMエミュレータ内部のエンベロープジェネレータ動作、特に「リセット直後からの最初の短いアタック」で顕著に現れる可能性が高い。
- 複数の仮説検証（高レート即時ゼロ無効、初期減衰強制、最初の数ステップ減速など）を `MP4M_YMFM_TEST_PERC` で実装し聴覚A/Bテストを実施。

**結果と判断**:
- どのテストモードでも「この曲のパーカッションが明確に改善し、かつ他の正常曲に悪影響が出ない」というバランスの取れた結果は得られなかった。
- ユーザーの一貫した原則（「他曲に影響を出す手法は却下」「この曲だけ特別扱いは意味がない」）を尊重し、ここで一旦妥協。
- 根本的なymfm/fmgen間のエンベロープ差を安全に埋める一般解は、現時点では見つからなかった。

**残した資産**:
- デバッグログ基盤の強化（HIGHRESモード、SetData直後INIT burst）は有用なためそのまま残す。将来的に再調査する際に活用可能。

**ロールバックしたもの**:
- すべての動作変更（TEST_PERC系の実験的エンベロープ挙動修正）は完全に撤回。他曲への影響を完全に排除。

**日付**: 2026-05-30
**結論**: バランスを優先し、現時点でこれ以上の修正は行わず、調査記録を残して一旦棚上げ。将来、ymfm側のより深い互換性改善や別アプローチが出た際に再検討する。
