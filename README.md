...
設定ループ回数を完了してファイルが終了する際の動作を選択できます：

| モード | 動作 | 用途 |
|--------|------|------|
| **NORMAL** | タイマー分母に到達 → フェードアウト → **停止** | 1曲ずつ手動で選択して再生したい場合 |
| **AUTO** | タイマー分母に到達 → フェードアウト → **次のファイル自動再生** | ファイルリスト順に連続再生したい場合 |
| **SHUFFLE** | タイマー分母に到達 → フェードアウト → **ランダムなファイル再生** | ファイルリストからランダムに再生したい場合 |

> **注**: タイマー分母とは、ループ設定で指定した回数です。例えば「ループ 2回」の場合、2回分の再生時間を完了するとタイマーに到達します。
...
| **ドキュメント表示 (.doc)** | 元の MP4M の機能だが、macOS 移植時に削除。テキスト表示で代替可能 |
| **ZMD/ZDF フォーマット対応** | ZMUSIC デコーダー実装が膨大。ユーザー需要も低い |
